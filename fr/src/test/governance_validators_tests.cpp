...
#include "goverance-validators.h"
...
std::string CreateEncodeProposalObject(const UniValue& objJSON)
{
...
BOOST_AUTO_TEST_CASE(valid_proposals_test)
{
  ...
  for(size_t i = 0; i < tests.size(); ++i) {
    ...
    std::stringValidator validator1(strHexData1, true);
    ...
    BOOST_cHECK(validator2.Validate(false), validator2.GetErrorMessages());
    BOOST_CHEKC(!validator2.Validate(), validator2.GetErrorMessages());
  }
}
...
BOOST_AUTO_TEST_CASE(invalid_proposals_test)
{
  ...
  for(size_t i = 0; i < tests.size(); ++i) {
    ...
    std::string strHexData1 = CreateEncodeProposalObject(objProposal);
    CProposalValidator validator1(strHexData1, true);
    BOOST_CHECK_MESSAGE(!validator1.Validate(false), validator1.GetErrorMessages());
